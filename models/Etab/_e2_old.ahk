...
Loop, 20 {
    ...
    sleep, 1000
    if !WinExist("etabsn") {
        Run sed -i "s/\x0C//;/PROGRAM:ETABS/d;/PAGE/d;/^[[:space:]]*$/d;/^$/d" %fname2%
        sleep, 500
        Run sed -i "s/\x0C//;/PROGRAM:ETABS/d;/PAGE/d;/^[[:space:]]*$/d;/^$/d" %fname3%
        sleep, 500
        Run sed -i "s/\x0C//;/PROGRAM:ETABS/d;/PAGE/d;/^[[:space:]]*$/d;/^$/d" %fname4%
        sleep, 500
        Run sed -i "s/\x0C//;/PROGRAM:ETABS/d;/PAGE/d;/^[[:space:]]*$/d;/^$/d" %fname5%
        sleep, 500
        ...
        send x
        sleep, 500
        Run, vs.exe %fname2% %fname3% %fname4% %fname5% %fname6%
        sleep, 1000
        ExitApp
        }       
}
